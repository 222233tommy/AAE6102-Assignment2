...
skymask=[];
if settings.useSkymask
skymask = table2array(readtable('skymask.csv'));  % 读入所有数据（包括表头）
skymask = skymask(2:end,:);
end
...
%% Find receiver position =================================================
    ...
    if size(activeChnList, 2) > 3
        ...
        cno = trackResults(activeChnList).CNo;
        ...
        [xyzdt,navSolutions.el(activeChnList, currMeasNr), ...
               ...
               navSolutions.raim(currMeasNr,:)] = ...
                       leastSquarePos(satPositions, clkCorrRawP, settings, cno,skymask);
        ...
        % clkCorrRawD = clkCorrRawD*(-1); % only for open-sky --sbs
...
%% Coordinate conversion ==================================================
        ...
        if currMeasNr == 1
            ...
            navSolutions.X_gt = X_gt;
            navSolutions.Y_gt = Y_gt;
            navSolutions.Z_gt = Z_gt; 
        ...
        navSolutions.error3d(currMeasNr) = sqrt((xyzdt(1)-X_gt).^2+(xyzdt(2)-Y_gt).^2+(xyzdt(3)-Z_gt).^2); 
