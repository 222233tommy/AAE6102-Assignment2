...
%% plot Standford chart --sbs

figure; 
subplot(1,3,1)
hold on
plot(navSolutions.raim(1,:), 'b', LineWidth=1); % sqrt WSSE
plot(navSolutions.raim(2,:), 'r', LineWidth=1); % Threshold
xlabel("Time (Epoch)")
ylabel("Test Statistics (m)")
title("Detection results for WLS")

subplot(1,3,2)
plot(navSolutions.raim(3,:), 'g', LineWidth=1); % PL
xlabel("Time (Epoch)")
ylabel("PL (m)")
title("PL for WLS")
...
err_3d = navSolutions.error3d;
pl_3d = navSolutions.raim(:,3);
subplot(1,3,3)
hold on;
